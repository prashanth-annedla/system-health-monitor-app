...
class LocalEventBus(EventBus):
    ...
    async def publish(self, event: HealthEvent) -> None:
        ...
        logger.info(f"Published event: Event published to local event bus",
...
async def process_event(
    ...
    if dag is None or event.component_id not in dag:
        logger.warning(f"Received event for unknown component: Event ignored",
            extra={"component_id": event.component_id, "status": event.status}
        )
        return
    previous_status = health_state.get(event.component_id, HealthStatus.UNKNOWN) 
    health_state[event.component_id] = event.status
    logger.info(f"Processed health event: Health status updated",
        extra={"component_id": event.component_id, "previous_status": previous_status, "new_status": event.status, "reason": event.details}
    )
