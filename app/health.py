...
async def evaluate_health(
    ...
    logger.info(
        f"Starting health evaluation for all components",
        extra={"evaluation_levels": len(levels)},
    )

    for level_idx, level in enumerate(levels):
        component_ids = list(level)
        logger.info(
            f"Evaluating health for components at level",
            extra={"level": level_idx, "components": component_ids},
        )
        ...
        for component_id, health_status in zip(component_ids, await healthresults):
            ...
            logger.info(
                f"Evaluated health for component",
                extra={"component_id": component_id, "status": health_status},
            )
        ...
        if health_state[component_id] == HealthStatus.UNHEALTHY:
            for dependent in get_direct_dependents(dag, component_id):
                if health_state[dependent] != HealthStatus.UNHEALTHY:
                    ...
                    logger.info(
                        f"Setting dependent component to DEGRADED due to unhealthy dependency",
                        extra={
                            "component_id": dependent,
                            "unhealthy_parent": component_id,
                        },
                    )


async def re_evaluate_dependents(
    dag: nx.DiGraph, health_state: Dict[str, HealthStatus], component_id: str
) -> None:
    if not nx.descendants(dag, component_id):
        logger.info(
            f"No dependents to re-evaluate for component",
            extra={"component_id": component_id},
        )
        return
    logger.info(
        f"Re-evaluating dependents for component", extra={"component_id": component_id}
    )
