...
from app.models import SystemRegisteredResponse, SystemInput, ComponentInput, HealthStatus, SystemHealthStatusResponse, HealthEvent
from typing import Dict, Optional
import networkx as nx
from app.events import create_event_bus, process_event, LocalEventBus
import asyncio
from app.dag import create_dag, ComponentNotFoundError, CycleDetectedError, get_dag_summary
import structlog
import logging
...
health_status: Dict[str, HealthStatus] = {}  # This will hold the health status of each component
...
async def register_component(
    ...
    health_status = {component.id: HealthStatus.HEALTHY for component in payload.components}
...
async def get_health_status() -> SystemHealthStatusResponse:
    ...
    """
    if not dag:
        raise HTTPException(status_code=503, detail="No components registered yet. POST /register_component to register components and trigger health evaluation.")
...
async def update_metrics(event: HealthEvent) -> dict:
    ...
    if event.component_id not in dag:
        raise HTTPException(status_code=404, detail=f"Component with id '{event.component_id}' not found in the registered system.")
    
    await process_event(event, dag, health_status)
    return {"accepted": True, "component_id": event.component_id, "new_status": event.status}