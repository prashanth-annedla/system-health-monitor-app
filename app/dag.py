    ...
    for component in Components:
        for id in component.dependencies:
            if id not in known_ids:
                raise ComponentNotFoundError(f"Component {component.id} has an unknown dependency: {id}")
            dagGraph.add_edge(id, component.id)

    if not nx.is_directed_acyclic_graph(dagGraph):
        raise CycleDetectedError("The provided components contain circular dependencies, which is not allowed in a DAG.")
